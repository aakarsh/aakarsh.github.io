...
* delcared in `<linux/slab.h>`
* preffered choice if allocations are not multiples of page size
...
* `__GFP_HIGHMEM` can use if needed
  ...
  * since logcial address cannot be returned for memory not mapped
...
* `GFP_KERNEL` most frequently used flag
  ...
  * can put caller to sleep , swap inactive pages to disk, flush diry pages,
